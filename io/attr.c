	...
	struct FTW		*data)
{
	...
	error = get_fsxattrat(AT_FDCWD, path, stat, &fsx);
	if (error) {
	...
	char		**argv)
{
	...
	error = get_fsxattrat(AT_FDCWD, name, &st, &fsx);
	if (error) {
	...
	struct FTW		*data)
{
	...
	error = get_fsxattrat(AT_FDCWD, path, stat, &attr);
	if (error) {
		fprintf(stderr, _("%s: cannot get flags on %s: %s\n"),
			progname, path, strerror(errno));
		exitcode = 1;
	...
	error = set_fsxattrat(AT_FDCWD, path, stat, &attr);
	if (error) {
	...
	char		**argv)
{
	...
	error = get_fsxattrat(AT_FDCWD, name, &st, &attr);
	if (error) {
		fprintf(stderr, _("%s: cannot get flags on %s: %s\n"),
			progname, name, strerror(errno));
		exitcode = 1;
	...
	attr.fsx_xflags |= orflags;
	attr.fsx_xflags &= ~andflags;
	error = set_fsxattrat(AT_FDCWD, name, &st, &attr);
	if (error) {
		fprintf(stderr, _("%s: cannot set flags on %s: %s\n"),
			progname, name, strerror(errno));
		exitcode = 1;
